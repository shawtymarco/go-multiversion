...
func marshalStackRequestItem(io *wireIO, value *protocol.StackRequestItem) {
	...
	if !io.reading {
		if io.runtime == nil {
			io.InvalidValue(value.Identifier, "stack request item identifier", "protocol 975 item mapping is not configured")
			return
		}
		...
		if !ok {
			io.InvalidValue(value.Identifier, "stack request item identifier", "identifier is absent from protocol 975")
			return
		}
		if blockRuntimeID > 0 {
	...
	if io.reading {
		...
		if !ok {
			io.InvalidValue(stack.NetworkID, "stack request item network ID", "unknown protocol 975 item")
			return
		}
...
func marshalItemUserData(io *wireIO, stack *protocol.ItemStack, empty bool) {
	if io.reading {
		...
		if stack.NetworkID == io.ShieldID() {
			var discardedBlockingTick int64
			reader.Int64(&discardedBlockingTick)
	...
	if stack.NetworkID == io.ShieldID() {
		var legacyBlockingTick int64
		writer.Int64(&legacyBlockingTick)
	}
	extra := buffer.Bytes()
	io.ByteSlice(&extra)
}
