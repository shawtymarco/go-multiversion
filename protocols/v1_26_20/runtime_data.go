...
import (
	...
	v975 "github.com/shawtymarco/go-multiversion/data/v975"
...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v975.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v975.Items()
	if err != nil {
		return nil, err
	}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 975 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
