...
func marshalClientMovementPredictionSync(io *wireIO, raw packet.Packet) {
	...
	// EntityDataFlagNotPickableFromInside was added after protocol 1001. Its
	// numeric index is therefore the protocol-1001 flag count.
...
func marshalInventoryContent(io *wireIO, raw packet.Packet) {
	...
	marshalItemInstanceNew(io, &pk.StorageItem)
}
...
func marshalMobArmourEquipment(io *wireIO, raw packet.Packet) {
	...
	marshalItemInstanceNew(io, &pk.Helmet)
	marshalItemInstanceNew(io, &pk.Chestplate)
	marshalItemInstanceNew(io, &pk.Leggings)
	marshalItemInstanceNew(io, &pk.Boots)
	marshalItemInstanceNew(io, &pk.Body)
}
