...
func marshalMobEquipment(io *wireIO, raw packet.Packet) {
	...
	io.Varuint64(&pk.EntityRuntimeID)
	marshalItem(io, &pk.NewItem.Stack)
...
func marshalMobArmourEquipment(io *wireIO, raw packet.Packet) {
	...
	io.Varuint64(&pk.EntityRuntimeID)
	marshalItem(io, &pk.Helmet.Stack)
	marshalItem(io, &pk.Chestplate.Stack)
	marshalItem(io, &pk.Leggings.Stack)
	marshalItem(io, &pk.Boots.Stack)
	if io.reading {
