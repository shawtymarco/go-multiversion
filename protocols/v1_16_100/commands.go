...
func marshalLegacyCommand(raw protocol.IO, command *protocol.Command) {
	...
	flags := uint8(command.Flags)
	io.Uint8(&flags)
	command.Flags = uint16(flags)
...
func legacyCommandParameterType(value uint32) uint32 {
	...
	mapped := map[uint32]uint32{
		protocol.CommandArgTypeInt: 1, protocol.CommandArgTypeFloat: 2,
		protocol.CommandArgTypeRValue: 3, protocol.CommandArgTypeWildcardInt: 4,
		protocol.CommandArgTypeOperator: 5, protocol.CommandArgTypeCompareOperator: 5,
		protocol.CommandArgTypeTarget: 6, protocol.CommandArgTypeStandaloneTarget: 6,
		protocol.CommandArgTypeWildcardTarget: 6, protocol.CommandArgTypeFilepath: 14,
		protocol.CommandArgTypeString: 29, protocol.CommandArgTypeBlockPosition: 37,
		protocol.CommandArgTypePosition: 37, protocol.CommandArgTypeMessage: 41,
		protocol.CommandArgTypeRawText: 43, protocol.CommandArgTypeJSON: 46,
		protocol.CommandArgTypeCommand: 53,
	}
...
func nativeCommandParameterType(value uint32) uint32 {
	...
	base := value & 0xfffff
	mapped := map[uint32]uint32{
		1: protocol.CommandArgTypeInt, 2: protocol.CommandArgTypeFloat, 3: protocol.CommandArgTypeRValue,
		4: protocol.CommandArgTypeWildcardInt, 5: protocol.CommandArgTypeOperator,
		6: protocol.CommandArgTypeTarget, 14: protocol.CommandArgTypeFilepath,
		29: protocol.CommandArgTypeString, 37: protocol.CommandArgTypePosition,
		41: protocol.CommandArgTypeMessage, 43: protocol.CommandArgTypeRawText,
		46: protocol.CommandArgTypeJSON, 53: protocol.CommandArgTypeCommand,
	}
