...
func marshalAddPlayer(io *wireIO, raw packet.Packet) {
	...
	marshalItem(io, &pk.HeldItem.Stack)
...
func marshalAddItemActor(io *wireIO, raw packet.Packet) {
	pk := raw.(*packet.AddItemActor)
	io.Varint64(&pk.EntityUniqueID)
	io.Varuint64(&pk.EntityRuntimeID)
	marshalItem(io, &pk.Item.Stack)
	io.Vec3(&pk.Position)
	io.Vec3(&pk.Velocity)
	io.EntityMetadata(&pk.EntityMetadata)
...
func marshalEntityMetadata(io *wireIO, metadata *protocol.EntityMetadata) {
	if io.reading {
		...
		decoded = upgradeEntityMetadataKeys(decoded)
	...
	encoded = downgradeEntityMetadataKeys(encoded)
...
func downgradeEntityFlags(metadata protocol.EntityMetadata) {
	...
	targetFirst, targetSecond := removeEntityFlagRange(uint64(first), uint64(second), protocol.EntityDataFlagDash, 95)
...
func upgradeEntityFlags(metadata protocol.EntityMetadata) {
	...
	first, _ := metadata[protocol.EntityDataKeyFlags].(int64)
	second, _ := metadata[protocol.EntityDataKeyFlagsTwo].(int64)
	nativeFirst, nativeSecond := insertEntityFlagRange(uint64(first), uint64(second), protocol.EntityDataFlagDash, 94)
...
func downgradeEntityMetadataKeys(metadata protocol.EntityMetadata) protocol.EntityMetadata {
	target := make(protocol.EntityMetadata, len(metadata))
...
func upgradeEntityMetadataKeys(metadata protocol.EntityMetadata) protocol.EntityMetadata {
	native := make(protocol.EntityMetadata, len(metadata))
	for key, value := range metadata {
		switch {
		case key <= 59:
			native[key] = value
...
func removeEntityFlagRange(first, second uint64, removed, maxCurrent int) (uint64, uint64) {
	var target [2]uint64
	for current := 0; current <= maxCurrent; current++ {
...
func insertEntityFlagRange(first, second uint64, inserted, maxLegacy int) (uint64, uint64) {
	var target [2]uint64
	for legacy := 0; legacy <= maxLegacy; legacy++ {
...
func legacyAbilityFlags(layers []protocol.AbilityLayer) (flags, actions uint32) {
	...
	if values&(protocol.AbilityBuild|protocol.AbilityMine) == protocol.AbilityBuild|protocol.AbilityMine {
		actions |= 1 << 0
	}
	permissions := []struct{ ability, action uint32 }{
		{protocol.AbilityDoorsAndSwitches, 1 << 1},
		{protocol.AbilityOpenContainers, 1 << 2},
		{protocol.AbilityAttackPlayers, 1 << 3},
		{protocol.AbilityAttackMobs, 1 << 4},
		{protocol.AbilityOperatorCommands, 1 << 5},
		{protocol.AbilityTeleport, 1 << 6},
	}
...
func abilityLayersFromLegacy(flags, actions uint32) []protocol.AbilityLayer {
	...
	permissions := []struct{ action, ability uint32 }{
		{1 << 1, protocol.AbilityDoorsAndSwitches},
		{1 << 2, protocol.AbilityOpenContainers},
		{1 << 3, protocol.AbilityAttackPlayers},
		{1 << 4, protocol.AbilityAttackMobs},
		{1 << 5, protocol.AbilityOperatorCommands},
		{1 << 6, protocol.AbilityTeleport},
	}
	for _, permission := range permissions {
