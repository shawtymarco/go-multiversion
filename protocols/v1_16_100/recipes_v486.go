...
func marshalCraftingDataV475(io *wireIO, raw packet.Packet) {
...
func marshalCraftingDataWrite(io *wireIO, pk *packet.CraftingData) {
	...
	for index := range pk.ShapelessRecipes {
		...
		marshalShapelessRecipeV475(io, &pk.ShapelessRecipes[index])
	}
	for index := range pk.ShapedRecipes {
		writeRecipeType(io, recipeShaped)
		marshalShapedRecipeV475(io, &pk.ShapedRecipes[index])
	}
	...
	for index := range pk.UserDataShapelessRecipes {
		...
		marshalShapelessRecipeV475(io, &pk.UserDataShapelessRecipes[index].ShapelessRecipe)
	}
}
...
func marshalCraftingDataRead(io *wireIO, pk *packet.CraftingData) {
	...
	for range count {
		...
		case recipeUserDataShapeless:
			...
			marshalShapelessRecipeV475(io, &recipe.ShapelessRecipe)
		...
		case recipeFurnace, recipeFurnaceData:
			marshalDiscardedFurnaceRecipeV475(io, recipeType == recipeFurnaceData)
		default:
...
func marshalShapedRecipeV475(io *wireIO, recipe *protocol.ShapedRecipe) {
	...
	for index := range recipe.Input {
		marshalRecipeIngredientV475(io, &recipe.Input[index])
	}
...
func marshalShapelessRecipeV475(io *wireIO, recipe *protocol.ShapelessRecipe) {
	io.String(&recipe.RecipeID)
	protocol.FuncIOSlice(io.directional(), &recipe.Input, func(raw protocol.IO, ingredient *protocol.ItemDescriptorCount) {
		marshalRecipeIngredientV475(asWireIO(raw), ingredient)
	})
	protocol.FuncSlice(io.directional(), &recipe.Output, io.Item)
	io.UUID(&recipe.UUID)
	io.String(&recipe.Block)
...
func marshalRecipeIngredientV475(io *wireIO, ingredient *protocol.ItemDescriptorCount) {
	var networkID int32
	var metadata int32
	if !io.reading {
		...
		case *protocol.DefaultItemDescriptor:
			if io.runtime == nil || io.runtime.currentItemMapper() == nil {
				io.InvalidValue(descriptor.Name, "recipe ingredient", "protocol 419 item mapping is not configured")
				return
			}
			var ok bool
			networkID, ok = io.runtime.currentItemMapper().TargetRuntimeID(descriptor.Name)
			if !ok {
				io.InvalidValue(descriptor.Name, "recipe ingredient", "item is absent from protocol 419")
				return
			}
			metadata = descriptor.MetadataValue
		default:
			io.InvalidValue(descriptor, "recipe ingredient", "descriptor has no protocol-419 numeric representation")
			return
		}
	}
	...
	if io.reading {
		if io.runtime == nil || io.runtime.currentItemMapper() == nil {
			io.InvalidValue(networkID, "recipe ingredient", "protocol 419 item mapping is not configured")
			return
		}
		name, ok := io.runtime.currentItemMapper().TargetIdentifier(networkID)
		if !ok {
			io.InvalidValue(networkID, "recipe ingredient", "unknown protocol-419 item")
			return
		}
...
func marshalDiscardedFurnaceRecipeV475(io *wireIO, withMetadata bool) {
	var networkID int32
	io.Varint32(&networkID)
	if withMetadata {
		var metadata int32
