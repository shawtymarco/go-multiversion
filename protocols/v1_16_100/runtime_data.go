...
import (
	...
	v419 "github.com/shawtymarco/go-multiversion/data/v419"
...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v419.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v419.Items()
	if err != nil {
		return nil, err
	}
	targetItems := make(map[string]mapping.TargetItem, len(historicalItems))
	for name, entry := range historicalItems {
		targetItems[name] = mapping.TargetItem{RuntimeID: int32(entry.RuntimeID)}
	}
	runtime := &runtimeData{
		blocks:        blocks,
		targetItems:   targetItems,
		creativeCount: 0,
		biomes:        v419.BiomeDefinitions(),
	}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 419 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
