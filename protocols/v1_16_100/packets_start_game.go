...
func marshalStartGame(io *wireIO, raw packet.Packet) {
	...
	targetSpawnBiomeType := pk.SpawnBiomeType
	if !io.reading {
		targetSpawnBiomeType += 3
	}
	io.Int16(&targetSpawnBiomeType)
	if io.reading {
		if targetSpawnBiomeType < 3 || targetSpawnBiomeType > 4 {
			io.UnknownEnumOption(targetSpawnBiomeType, "spawn biome type")
			return
		}
		pk.SpawnBiomeType = targetSpawnBiomeType - 3
	}
	...
	if io.reading {
		pk.EducationSharedResourceURI = protocol.EducationSharedResourceURI{}
	}
...
func marshalGameRule(raw protocol.IO, rule *protocol.GameRule) {
	...
	io.String(&rule.Name)
	if io.reading {
		rule.CanBeModifiedByPlayer = false
	}
	var kind uint32
	if !io.reading {
...
func marshalPlayerMovementSettings(io *wireIO, settings *protocol.PlayerMovementSettings) {
	...
	if io.reading {
		...
		settings.ServerAuthoritativeBlockBreaking = false
	}
}
...
func marshalStartGameItems(io *wireIO) {
	...
	if !io.reading {
		if io.runtime == nil || io.runtime.currentItemMapper() == nil {
			io.InvalidValue(nil, "start game items", "protocol 419 item mapping is not configured")
			return
		}
...
func marshalBiomeDefinitionList(io *wireIO, _ packet.Packet) {
	...
	if io.runtime == nil || len(io.runtime.biomes) == 0 {
		io.InvalidValue(nil, "biome definitions", "protocol 419 biome data is not configured")
		return
	}
