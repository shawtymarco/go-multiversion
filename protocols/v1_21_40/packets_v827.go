...
func marshalCameraInstruction(io *wireIO, raw packet.Packet) {
	...
	protocol.OptionalFunc(io.directional(), &pk.Set, func(set *protocol.CameraInstructionSet) {
		io.Uint32(&set.Preset)
		protocol.OptionalMarshaler(io.directional(), &set.Ease)
		protocol.OptionalFunc(io.directional(), &set.Position, io.Vec3)
		protocol.OptionalFunc(io.directional(), &set.Rotation, io.Vec2)
		protocol.OptionalFunc(io.directional(), &set.Facing, io.Vec3)
		protocol.OptionalFunc(io.directional(), &set.ViewOffset, io.Vec2)
		protocol.OptionalFunc(io.directional(), &set.EntityOffset, io.Vec3)
		protocol.OptionalFunc(io.directional(), &set.Default, io.Bool)
		if io.reading {
			set.IgnoreStartingValuesComponent = false
		}
	})
	protocol.OptionalFunc(io.directional(), &pk.Clear, io.Bool)
	protocol.OptionalMarshaler(io.directional(), &pk.Fade)
	protocol.OptionalFunc(io.directional(), &pk.Target, func(target *protocol.CameraInstructionTarget) {
		protocol.OptionalFunc(io.directional(), &target.CenterOffset, io.Vec3)
		io.Int64(&target.EntityUniqueID)
	})
	protocol.OptionalFunc(io.directional(), &pk.RemoveTarget, io.Bool)
	if io.reading {
		pk.FieldOfView = protocol.Optional[protocol.CameraInstructionFieldOfView]{}
...
func marshalCameraAimAssistPresets(io *wireIO, raw packet.Packet) {
	...
	groups := [][]protocol.CameraAimAssistCategory(nil)
	if !io.reading && len(pk.Categories) != 0 {
		groups = [][]protocol.CameraAimAssistCategory{pk.Categories}
	}
	protocol.FuncIOSlice(io.directional(), &groups, func(raw protocol.IO, categories *[]protocol.CameraAimAssistCategory) {
		legacy := asWireIO(raw)
		identifier := "default"
		legacy.String(&identifier)
		protocol.FuncIOSlice(legacy.directional(), categories, func(raw protocol.IO, category *protocol.CameraAimAssistCategory) {
			categoryIO := asWireIO(raw)
			categoryIO.String(&category.Name)
			marshalCameraAimAssistPriorities827(categoryIO, &category.Priorities)
		})
	})
	protocol.FuncIOSlice(io.directional(), &pk.Presets, func(raw protocol.IO, preset *protocol.CameraAimAssistPreset) {
		legacy := asWireIO(raw)
		legacy.String(&preset.Identifier)
		categoryGroup := "default"
		legacy.String(&categoryGroup)
		protocol.FuncSlice(legacy.directional(), &preset.BlockExclusions, legacy.String)
		protocol.FuncSlice(legacy.directional(), &preset.LiquidTargets, legacy.String)
		protocol.Slice(legacy.directional(), &preset.ItemSettings)
		protocol.OptionalFunc(legacy.directional(), &preset.DefaultItemSettings, legacy.String)
		protocol.OptionalFunc(legacy.directional(), &preset.HandSettings, legacy.String)
	})
	if io.reading {
		pk.Categories = nil
		for _, categories := range groups {
			pk.Categories = append(pk.Categories, categories...)
		}
		pk.Operation = 0
	}
}
