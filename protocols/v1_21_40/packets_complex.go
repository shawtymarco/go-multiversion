...
func marshalPlayerAuthInput(io *wireIO, raw packet.Packet) {
	...
	io.Float32(&pk.HeadYaw)
	marshalInputFlags748(io, &pk.InputData)
	...
	if io.reading {
		pk.RawMoveVector = mgl32.Vec2{}
	}
}

func marshalInputFlags748(io *wireIO, flags *protocol.InputFlags) {
	var legacy uint64
	if !io.reading {
		for index := 0; index <= packet.InputFlagDownRight; index++ {
			if index < flags.Len() && flags.Load(index) {
				legacy |= uint64(1) << index
			}
		}
		for index := packet.InputFlagCameraRelativeMovementEnabled; index <= packet.InputFlagStopSpinAttack; index++ {
			if index < flags.Len() && flags.Load(index) {
				legacy |= uint64(1) << (index - 1)
			}
		}
	}
	io.Varuint64(&legacy)
	if io.reading {
		*flags = protocol.NewInputFlags(65)
		for index := 0; index <= packet.InputFlagDownRight; index++ {
			if legacy&(uint64(1)<<index) != 0 {
				flags.Set(index)
			}
		}
		for index := packet.InputFlagCameraRelativeMovementEnabled; index <= packet.InputFlagStopSpinAttack; index++ {
			if legacy&(uint64(1)<<(index-1)) != 0 {
				flags.Set(index)
			}
		}
	}
