// Package v1_21_40 implements the Minecraft Bedrock 1.21.5x protocol 748
...
const (
	...
	ID int32 = 748
	// Version is the newest stable Minecraft version in the protocol 748 family.
	Version = "1.21.44"
)

// Protocol implements minecraft.Protocol for stable Minecraft 1.21.40 through
// 1.21.44. Registry-aware consumers may advertise it after construction.
type Protocol struct {
	runtime *runtimeData
}

// MappingReport summarises the exact registries and explicit fallbacks used by
// a configured protocol-748 adapter.
...
// New creates a protocol 748 wire adapter.
func New() minecraft.Protocol { return &Protocol{} }

// NewWithBlockRegistry creates a fully configured protocol-748 adapter using
...
// MapBlockRuntimeID maps a current native runtime ID to the protocol-748
...
func (Protocol) Packets(listener bool) packet.Pool {
	...
	if listener {
		...
		delete(pool, packet.IDStructureTemplateDataRequest)
	...
	delete(pool, packet.IDCameraAimAssistPresets)
