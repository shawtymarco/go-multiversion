...
type runtimeData struct {
	...
	creative    []v748.CreativeItem
	furnaces    []v748.FurnaceRecipe
...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v748.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v748.Items()
	if err != nil {
		return nil, err
	}
	...
	creative, err := v748.Creative()
	if err != nil {
		return nil, err
	}
	furnaces, err := v748.Furnaces()
	if err != nil {
		return nil, err
	}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 748 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
