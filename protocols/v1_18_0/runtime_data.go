...
import (
	...
	v475 "github.com/shawtymarco/go-multiversion/data/v475"
...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v475.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v475.Items()
	if err != nil {
		return nil, err
	}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 475 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
