...
func marshalRecipeIngredientV475(io *wireIO, ingredient *protocol.ItemDescriptorCount) {
	...
	if !io.reading {
		...
		case *protocol.DefaultItemDescriptor:
			if io.runtime == nil || io.runtime.currentItemMapper() == nil {
				io.InvalidValue(descriptor.Name, "recipe ingredient", "protocol 475 item mapping is not configured")
				return
			}
			var ok bool
			networkID, ok = io.runtime.currentItemMapper().TargetRuntimeID(descriptor.Name)
			if !ok {
				io.InvalidValue(descriptor.Name, "recipe ingredient", "item is absent from protocol 475")
				return
			}
			metadata = descriptor.MetadataValue
		default:
			io.InvalidValue(descriptor, "recipe ingredient", "descriptor has no protocol-475 numeric representation")
			return
		}
	}
	...
	if io.reading {
		if io.runtime == nil || io.runtime.currentItemMapper() == nil {
			io.InvalidValue(networkID, "recipe ingredient", "protocol 475 item mapping is not configured")
			return
		}
		name, ok := io.runtime.currentItemMapper().TargetIdentifier(networkID)
		if !ok {
			io.InvalidValue(networkID, "recipe ingredient", "unknown protocol-475 item")
			return
		}
