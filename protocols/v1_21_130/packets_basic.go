...
func marshalClientMovementPredictionSync(io *wireIO, raw packet.Packet) {
	...
	// EntityDataFlagUsesLegacyFriction was added after protocol 898. Its
	// numeric index is therefore the protocol-898 flag count.
	io.Bitset(&pk.ActorFlags, protocol.EntityDataFlagUsesLegacyFriction)
...
func marshalPartyChanged(io *wireIO, raw packet.Packet) {
	pk := raw.(*packet.PartyChanged)
	var partyID string
	if info, ok := pk.PartyInfo.Value(); ok {
...
func marshalPlayerEnchantOptions(io *wireIO, raw packet.Packet) {
	...
	protocol.FuncIOSlice(io.directional(), &pk.Options, func(raw protocol.IO, option *protocol.EnchantmentOption) {
		...
		if cost > 255 {
			legacy.InvalidValue(cost, "enchantment cost", "exceeds current model")
			return
		}
		option.Cost = uint8(cost)
		protocol.Single(raw, &option.Enchantments)
...
func marshalStructureBlockUpdate(io *wireIO, raw packet.Packet) {
	...
	marshalStructureSettings898(io, &pk.Settings)
