...
func wireFixtures() []packetFixture {
	return []packetFixture{
		...
		{name: "full/text_chat", listener: true, new: func() packet.Packet {
			return &packet.Text{TextType: packet.TextTypeChat, SourceName: "player", Message: "hello", XUID: "1"}
		}},
		{name: "full/text_translation", new: func() packet.Packet {
			return &packet.Text{TextType: packet.TextTypeTranslation, Message: "translation.key", Parameters: []string{"value"}}
		}},
		{name: "full/book_edit", listener: true, new: func() packet.Packet {
			return &packet.BookEdit{ActionType: packet.BookActionAddPage, InventorySlot: 1, PageNumber: 2, Text: "page", PhotoName: "photo"}
		}},
		{name: "full/add_volume_entity", new: func() packet.Packet {
			return &packet.AddVolumeEntity{EntityRuntimeID: 1, EntityMetadata: map[string]any{"name": "volume"}, EncodingIdentifier: "test:volume", InstanceIdentifier: "fog", Bounds: [2]protocol.BlockPos{{1, 2, 3}, {4, 5, 6}}, Dimension: 0, EngineVersion: "1.21.130"}
		}},
...
func TestProtocolIdentityAndPools(t *testing.T) {
	...
	if got, want := protocolVersion.ID(), int32(898); got != want {
		t.Fatalf("protocol ID: got %d, want %d", got, want)
	}
	if got, want := protocolVersion.Ver(), "1.21.132"; got != want {
		t.Fatalf("version: got %q, want %q", got, want)
	}
	if _, ok := protocolVersion.Packets(false)[packet.IDServerPlayerPostMovePosition]; ok {
		t.Fatal("ServerPlayerPostMovePosition is present in protocol 898 server pool")
	}
	if _, ok := protocolVersion.Packets(true)[packet.IDUpdateBlock]; ok {
		t.Fatal("UpdateBlock is present in protocol 898 client pool")
	}
	for _, id := range []uint32{
		...
		packet.IDClientBoundDataDrivenUIShowScreen, packet.IDClientBoundDataDrivenUICloseScreen,
		packet.IDClientBoundDataDrivenUIReload, packet.IDClientBoundTextureShift, packet.IDVoxelShapes,
		packet.IDCameraSpline, packet.IDCameraAimAssistActorPriority,
	} {
		if _, ok := protocolVersion.Packets(false)[id]; ok {
			t.Fatalf("post-898 packet ID %d is present in protocol 898 server pool", id)
		}
		if _, ok := protocolVersion.Packets(true)[id]; ok {
			t.Fatalf("post-898 packet ID %d is present in protocol 898 client pool", id)
		}
	}
}
...
func TestHistoricalGophertunnelOracle(t *testing.T) {
	oracle := os.Getenv("V898_WIRE_ORACLE")
	if oracle == "" {
		t.Skip("V898_WIRE_ORACLE is not set")
	}
	for _, fixture := range wireFixtures() {
		t.Run(fixture.name, func(t *testing.T) {
			pk := fixture.new()
