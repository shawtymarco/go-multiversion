...
// operations whose protocol-898 layout differs from native.
type wireIO struct {
	protocol.IO
	reading bool
	runtime *runtimeData
...
func (io *wireIO) SoundPos(position *mgl32.Vec3) {
	blockPosition := protocol.BlockPos{int32(position[0] * 8), int32(position[1] * 8), int32(position[2] * 8)}
	uBlockPos(io, &blockPosition)
	*position = mgl32.Vec3{float32(blockPosition[0]) / 8, float32(blockPosition[1]) / 8, float32(blockPosition[2]) / 8}
}
