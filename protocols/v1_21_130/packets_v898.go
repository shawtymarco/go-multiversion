...
func marshalGraphicsOverrideParameter(io *wireIO, raw packet.Packet) {
	...
	io.Bool(&pk.Reset)
	if io.reading {
		pk.FloatValue = protocol.Optional[float32]{}
		pk.Vec3Value = protocol.Optional[mgl32.Vec3]{}
...
func marshalClientBoundDataStore(io *wireIO, raw packet.Packet) {
	...
	protocol.FuncIOSlice(io.directional(), &pk.Updates, func(raw protocol.IO, entry *protocol.DataStoreChangeEntry) {
		...
		case protocol.DataStoreChangeTypeUpdate:
			marshalDataStoreUpdate898(legacy, &entry.Update)
		case protocol.DataStoreChangeTypeChange:
			...
			marshalDataStorePropertyValue898(legacy, &entry.Change.NewValue)
...
func marshalDataStoreUpdate898(io *wireIO, value *protocol.DataStoreUpdate) {
...
func marshalServerBoundDataStore(io *wireIO, raw packet.Packet) {
	marshalDataStoreUpdate898(io, &raw.(*packet.ServerBoundDataStore).Update)
}

func marshalDataStorePropertyValue898(io *wireIO, value *protocol.DataStorePropertyValue) {
	io.Int32(&value.Type)
	switch value.Type {
	...
	case protocol.DataStorePropertyTypeMap:
		protocol.FuncIOSlice(io.directional(), &value.MapValue, func(raw protocol.IO, entry *protocol.DataStoreMapEntry) {
			...
			marshalDataStorePropertyValue898(legacy, &entry.Value)
		})
	default:
...
func marshalClientBoundAttributeLayerSync(io *wireIO, raw packet.Packet) {
	...
	case protocol.AttributeLayerPayloadTypeUpdateEnvironment:
		...
		protocol.FuncIOSlice(io.directional(), &pk.EnvironmentAttributes, func(raw protocol.IO, value *protocol.EnvironmentAttributeData) {
			marshalEnvironmentAttribute898(asWireIO(raw), value)
		})
...
func marshalAttributeLayer898(io *wireIO, layer *protocol.AttributeLayerData) {
	io.String(&layer.Name)
	io.Varint32(&layer.DimensionID)
	marshalAttributeLayerSettings898(io, &layer.Settings)
...
func marshalAttributeLayerSettings898(io *wireIO, settings *protocol.AttributeLayerSettings) {
...
func marshalEnvironmentAttribute898(io *wireIO, value *protocol.EnvironmentAttributeData) {
