...
func marshalEntityMetadata(io *wireIO, metadata *protocol.EntityMetadata) {
	...
	for key := range *metadata {
		if key >= protocol.EntityDataKeyAimAssistPriorityPresetID {
			continue
		}
	...
	for _, rawKey := range keys {
		...
		io.Varuint32(&key)
		value := (*metadata)[key]
		if key == protocol.EntityDataKeyFlagsTwo {
			if flags, ok := value.(int64); ok {
				bitCount := uint(protocol.EntityDataFlagUsesLegacyFriction - 64)
				if bitCount < 64 {
					flags &= int64((uint64(1) << bitCount) - 1)
				}
				value = flags
			}
		}
		writeEntityMetadataValue(io, value)
	}
}
...
func marshalAbilityData(io *wireIO, data *protocol.AbilityData) {
	...
	for i := range data.Layers {
		data.Layers[i].Marshal(io.directional())
	}
}
...
func marshalPlayerListEntry(io protocol.IO, entry *protocol.PlayerListEntry) {
	...
	marshalLegacyARGB(legacy, &entry.PlayerColour)
}
...
func marshalSubChunkEntry(io *wireIO, entry *protocol.SubChunkEntry, cacheEnabled bool) {
	...
	if entry.RenderHeightMapType == protocol.HeightMapDataHasData {
		data, _ := entry.RenderHeightMapData.Value()
		protocol.FuncSliceOfLen(io.directional(), 256, &data, io.Int8)
		entry.RenderHeightMapData = protocol.Option(data)
	}
	if cacheEnabled {
...
func marshalShapeData(io *wireIO, value *protocol.ShapeData) {
	...
	if !io.reading {
		...
		default:
			io.UnknownEnumOption(fmt.Sprintf("%T", *value), "debug shape data type")
			return
		}
	}
