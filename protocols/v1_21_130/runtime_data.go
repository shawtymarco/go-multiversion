...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v898.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v898.Items()
	if err != nil {
		return nil, err
	}
	...
	creative, err := v898.Creative()
	if err != nil {
		return nil, err
	}
	historicalFurnace, err := v898.Furnace()
	if err != nil {
		return nil, err
	}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 898 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
