...
func wireFixtures() []packetFixture {
	return []packetFixture{
		...
		{name: "full/client_movement_prediction_sync", listener: true, new: func() packet.Packet {
		...
		{name: "full/server_bound_pack_setting_change", listener: true, new: func() packet.Packet {
		...
		{name: "v844/resource_pack_stack", new: func() packet.Packet {
			return &packet.ResourcePackStack{TexturePackRequired: true, TexturePacks: []protocol.StackResourcePack{{}}, BaseGameVersion: "1.21.114", Experiments: []protocol.ExperimentData{{Name: "test", Enabled: true}}, ExperimentsPreviouslyToggled: true, IncludeEditorPacks: true}
		}},
		{name: "v844/actor_event", new: func() packet.Packet {
		...
		{name: "v844/mob_effect", new: func() packet.Packet {
		...
		{name: "v844/interact", listener: true, new: func() packet.Packet {
		...
		{name: "v844/command_request", listener: true, new: func() packet.Packet {
		...
		{name: "v844/camera_instruction_fov", new: func() packet.Packet {
			return &packet.CameraInstruction{FieldOfView: protocol.Option(protocol.CameraInstructionFieldOfView{FieldOfView: 70, EaseTime: 1, EaseType: protocol.EasingTypeInOutQuad, Clear: true})}
		}},
		{name: "v844/update_client_options", listener: true, new: func() packet.Packet {
			return &packet.UpdateClientOptions{GraphicsMode: protocol.Option(byte(packet.GraphicsModeFancy)), FilterProfanity: protocol.Option(true)}
		}},
		{name: "v844/camera_aim_assist_presets", new: func() packet.Packet {
			return &packet.CameraAimAssistPresets{Categories: []protocol.CameraAimAssistCategory{{Name: "category", Priorities: protocol.CameraAimAssistPriorities{Entities: []protocol.CameraAimAssistPriority{{Identifier: "minecraft:pig", Priority: 1}}, Blocks: []protocol.CameraAimAssistPriority{{Identifier: "minecraft:stone", Priority: 2}}, EntityDefault: protocol.Option(int32(3)), BlockDefault: protocol.Option(int32(4))}}}, Presets: []protocol.CameraAimAssistPreset{{Identifier: "preset", BlockExclusions: []string{"minecraft:air"}, LiquidTargets: []string{"minecraft:cod"}, ItemSettings: []protocol.CameraAimAssistItemSettings{{Item: "minecraft:stick", Category: "category"}}, DefaultItemSettings: protocol.Option("category"), HandSettings: protocol.Option("category")}}, Operation: packet.CameraAunAssistPresetOperationSet}
		}},
		{name: "v844/player_enchant_options", new: func() packet.Packet {
			return &packet.PlayerEnchantOptions{Options: []protocol.EnchantmentOption{{Cost: 30, Name: "enchant", RecipeNetworkID: 7}}}
		}},
		{name: "v844/level_sound_event", new: func() packet.Packet {
		...
		{name: "v844/book_edit", listener: true, new: func() packet.Packet {
			return &packet.BookEdit{ActionType: packet.BookActionSwapPages, InventorySlot: 2, PageNumber: 3, SecondaryPageNumber: 4}
		}},
		{name: "v844/client_cache_blob_status", listener: true, new: func() packet.Packet {
			return &packet.ClientCacheBlobStatus{MissHashes: []uint64{1, 2}, HitHashes: []uint64{3}}
		}},
		{name: "v844/structure_template", listener: true, new: func() packet.Packet {
			return &packet.StructureTemplateDataRequest{StructureName: "test", Position: protocol.BlockPos{-1, 64, 2}, Settings: protocol.StructureSettings{PaletteName: "default", Size: protocol.BlockPos{3, 4, 5}, Offset: protocol.BlockPos{-1, 2, 3}, Integrity: 1}, RequestType: 1}
		}},
		{name: "v844/biome_definition_list", new: func() packet.Packet {
			return &packet.BiomeDefinitionList{BiomeDefinitions: []protocol.BiomeDefinition{{NameIndex: 1, BiomeID: -1, Temperature: 0.5, Downfall: 0.25, Rain: true, Tags: protocol.Option([]uint16{2}), ChunkGeneration: protocol.Option(protocol.BiomeChunkGeneration{SurfaceMaterials: protocol.Option(protocol.BiomeSurfaceMaterial{TopBlock: 1, MidBlock: 2, SeaFloorBlock: 3, FoundationBlock: 4, SeaBlock: 5, SeaFloorDepth: 6}), HasDefaultOverworldSurface: true})}}, StringList: []string{"plains", "overworld"}}
		}},
		{name: "v844/event_cauldron", new: func() packet.Packet {
...
func TestProtocolIdentityAndPools(t *testing.T) {
	...
	if got, want := protocolVersion.ID(), int32(844); got != want {
		t.Fatalf("protocol ID: got %d, want %d", got, want)
	}
	if got, want := protocolVersion.Ver(), "1.21.114"; got != want {
		t.Fatalf("version: got %q, want %q", got, want)
	}
	if _, ok := protocolVersion.Packets(false)[packet.IDServerPlayerPostMovePosition]; ok {
		t.Fatal("ServerPlayerPostMovePosition is present in protocol 844 server pool")
	}
	if _, ok := protocolVersion.Packets(true)[packet.IDUpdateBlock]; ok {
		t.Fatal("UpdateBlock is present in protocol 844 client pool")
	}
	if got, want := len(protocolVersion.Packets(false)), 210; got != want {
		t.Fatalf("server pool size: got %d, want %d (historical 211 minus unsupported ServerScriptDebugDrawer)", got, want)
	}
	if got, want := len(protocolVersion.Packets(true)), 79; got != want {
		t.Fatalf("client pool size: got %d, want historical %d", got, want)
	}
	if _, ok := protocolVersion.Packets(true)[packet.IDServerBoundPackSettingChange]; !ok {
		t.Fatal("ServerBoundPackSettingChange is absent from protocol 844 client pool")
	}
...
func TestStableGameVersions(t *testing.T) {
	for _, version := range []string{"1.21.110", "1.21.111", "1.21.112", "1.21.113", "1.21.114"} {
		if !isStableGameVersion(version) {
			t.Fatalf("stable family version %s was rejected", version)
		}
	}
	for _, version := range []string{"1.21.109", "1.21.115", "1.21.120", "1.26.45", ""} {
		if isStableGameVersion(version) {
...
func TestHistoricalGophertunnelOracle(t *testing.T) {
	oracle := os.Getenv("V844_WIRE_ORACLE")
	if oracle == "" {
		t.Skip("V844_WIRE_ORACLE is not set")
	}
	for _, fixture := range wireFixtures() {
		t.Run(fixture.name, func(t *testing.T) {
			pk := fixture.new()
