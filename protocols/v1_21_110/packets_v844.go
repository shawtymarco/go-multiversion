...
func marshalCommandRequest(io *wireIO, raw packet.Packet) {
	...
	marshalCommandOrigin844(io, &pk.CommandOrigin)
	io.Bool(&pk.Internal)
...
func marshalCommandOutput(io *wireIO, raw packet.Packet) {
	...
	protocol.FuncIOSlice(io.directional(), &pk.OutputMessages, marshalCommandOutputMessage844)
...
func marshalCommandOrigin844(io *wireIO, origin *protocol.CommandOrigin) {
...
func marshalCommandOutputMessage844(raw protocol.IO, message *protocol.CommandOutputMessage) {
	io := asWireIO(raw)
...
func marshalEvent(io *wireIO, raw packet.Packet) {
	...
	marshalEventData844(io, pk.Event)
}

func marshalEventData844(io *wireIO, event protocol.Event) {
...
func marshalCameraAimAssistPresets(io *wireIO, raw packet.Packet) {
	...
	protocol.FuncIOSlice(io.directional(), &pk.Categories, func(raw protocol.IO, category *protocol.CameraAimAssistCategory) {
		...
		marshalCameraAimAssistPriorities844(legacy, &category.Priorities)
	})
	protocol.FuncIOSlice(io.directional(), &pk.Presets, marshalCameraAimAssistPreset844)
	io.Uint8(&pk.Operation)
}

func marshalCameraAimAssistPriorities844(io *wireIO, priorities *protocol.CameraAimAssistPriorities) {
...
func marshalCameraAimAssistPreset844(raw protocol.IO, preset *protocol.CameraAimAssistPreset) {
	io := asWireIO(raw)
