...
func marshalStackRequestItem(io *wireIO, value *protocol.StackRequestItem) {
	...
	if !io.reading {
		if io.runtime == nil {
			io.InvalidValue(value.Identifier, "stack request item identifier", "protocol 844 item mapping is not configured")
			return
		}
		...
		if !ok {
			io.InvalidValue(value.Identifier, "stack request item identifier", "identifier is absent from protocol 844")
			return
		}
		if blockRuntimeID > 0 {
	...
	if io.reading {
		...
		if !ok {
			io.InvalidValue(stack.NetworkID, "stack request item network ID", "unknown protocol 844 item")
			return
		}
		...
		if stack.BlockRuntimeID > 0 {
			...
			if !ok {
				io.InvalidValue(stack.BlockRuntimeID, "stack request block runtime ID", "unknown protocol 844 block")
				return
			}
