...
func marshalUpdateSubChunkBlocks(io *wireIO, raw packet.Packet) {
	...
	io.UBlockPos(&pk.Position)
	protocol.FuncIOSlice(io.directional(), &pk.Blocks, marshalBlockChangeEntry844)
	protocol.FuncIOSlice(io.directional(), &pk.Extra, marshalBlockChangeEntry844)
}

func marshalBlockChangeEntry844(raw protocol.IO, entry *protocol.BlockChangeEntry) {
	io := asWireIO(raw)
...
func marshalStructureSettings844(io *wireIO, settings *protocol.StructureSettings) {
