...
import (
	...
	v844 "github.com/shawtymarco/go-multiversion/data/v844"
...
type runtimeData struct {
	...
	creative    v844.CreativeData
	furnaces    []v844.FurnaceRecipe
...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v844.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v844.Items()
	if err != nil {
		return nil, err
	}
	...
	creative, err := v844.Creative()
	if err != nil {
		return nil, err
	}
	furnaces, err := v844.Furnaces()
	if err != nil {
		return nil, err
	}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 844 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
