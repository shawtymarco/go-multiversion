...
func marshalClientMovementPredictionSync(io *wireIO, raw packet.Packet) {
	...
	legacyFlags := protocol.NewBitset(entityDataFlagCount844)
	if !io.reading {
		limit := min(pk.ActorFlags.Len(), entityDataFlagCount844)
	...
	io.Bitset(&legacyFlags, entityDataFlagCount844)
	if io.reading {
		pk.ActorFlags = protocol.NewBitset(protocol.EntityDataFlagCount)
		for index := 0; index < entityDataFlagCount844; index++ {
...
func marshalDimensionData(io *wireIO, raw packet.Packet) {
	...
	protocol.FuncIOSlice(io.directional(), &pk.Definitions, func(raw protocol.IO, definition *protocol.DimensionDefinition) {
		...
		if legacy.reading {
			definition.MinimumY, definition.HeightRange, definition.DimensionType = minimumY, maximumY-minimumY, 0
		}
	})
}
...
func marshalStructureBlockUpdate(io *wireIO, raw packet.Packet) {
	...
	marshalStructureSettings844(io, &pk.Settings)
...
func marshalSubChunkRequest(io *wireIO, raw packet.Packet) {
	...
	protocol.SliceUint32Length(io.directional(), &pk.Offsets)
}
