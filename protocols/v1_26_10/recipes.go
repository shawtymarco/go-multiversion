...
import (
	...
	v944 "github.com/shawtymarco/go-multiversion/data/v944"
...
func buildLegacyFurnaceRecipes(source []v944.FurnaceRecipe, targetItems map[string]mapping.TargetItem, blocks *mapping.BlockMapper) ([]legacyFurnaceRecipe, error) {
	recipes := make([]legacyFurnaceRecipe, len(source))
	for index, recipe := range source {
		input, ok := targetItems[recipe.Input.Name]
		if !ok {
			return nil, fmt.Errorf("furnace recipe %d input item %q is absent from protocol 944", index, recipe.Input.Name)
		}
		output, ok := targetItems[recipe.Output.Name]
		if !ok {
			return nil, fmt.Errorf("furnace recipe %d output item %q is absent from protocol 944", index, recipe.Output.Name)
		}
		...
		if recipe.Output.State.Name != "" {
			...
			if !ok {
				return nil, fmt.Errorf("furnace recipe %d output block %q is absent from protocol 944", index, recipe.Output.State.Name)
			}
