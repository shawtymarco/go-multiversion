...
type runtimeData struct {
	...
	creative    v944.CreativeData
	furnace     []legacyFurnaceRecipe
...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v944.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v944.Items()
	if err != nil {
		return nil, err
	}
	...
	creative, err := v944.Creative()
	if err != nil {
		return nil, err
	}
	historicalFurnace, err := v944.Furnace()
	if err != nil {
		return nil, err
	}
	furnace, err := buildLegacyFurnaceRecipes(historicalFurnace, targetItems, blocks)
	if err != nil {
		return nil, err
	}
	runtime := &runtimeData{blocks: blocks, targetItems: targetItems, creative: creative, furnace: furnace}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 944 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
