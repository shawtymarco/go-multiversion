...
func wireFixtures() []packetFixture {
	return []packetFixture{
		...
		{name: "party_changed", listener: true, new: func() packet.Packet {
			return &packet.PartyChanged{PartyInfo: protocol.Option(packet.PartyInfo{PartyID: "party"})}
		}},
		...
		{name: "full/client_bound_data_store_update", new: func() packet.Packet {
			return &packet.ClientBoundDataStore{Updates: []protocol.DataStoreChangeEntry{{ChangeType: protocol.DataStoreChangeTypeUpdate, Update: protocol.DataStoreUpdate{DataStoreName: "store", Property: "value", Path: "path", ControlType: protocol.DataStoreControlDouble, DoubleValue: 1.5, PropertyUpdateCount: 2, PathUpdateCount: 3}}}}
		}},
		...
		{name: "full/locator_bar", new: func() packet.Packet {
			return &packet.LocatorBar{Waypoints: []protocol.LocatorBarWaypoint{{
				GroupHandle: uuid.MustParse("22222222-3333-4444-5555-666666666666"), Action: protocol.WaypointActionAdd,
				Waypoint: protocol.Waypoint{UpdateFlag: protocol.WaypointUpdateFlagVisible | protocol.WaypointUpdateFlagTextureID, Visible: protocol.Option(true), TexturePath: protocol.Option("textures/ui/locator_bar/square"), IconSize: protocol.Option(mgl32.Vec2{8, 8})},
			}}}
		}},
		...
		{name: "full/server_bound_data_store", listener: true, new: func() packet.Packet {
			return &packet.ServerBoundDataStore{Update: protocol.DataStoreUpdate{DataStoreName: "store", Property: "flag", Path: "path", ControlType: protocol.DataStoreControlBoolean, BoolValue: true, PropertyUpdateCount: 1, PathUpdateCount: 2}}
		}},
...
func TestProtocolIdentityAndPools(t *testing.T) {
	...
	if got, want := protocolVersion.ID(), int32(944); got != want {
		t.Fatalf("protocol ID: got %d, want %d", got, want)
	}
	if got, want := protocolVersion.Ver(), "1.26.14"; got != want {
		t.Fatalf("version: got %q, want %q", got, want)
	}
	if _, ok := protocolVersion.Packets(false)[packet.IDServerPlayerPostMovePosition]; ok {
		t.Fatal("ServerPlayerPostMovePosition is present in protocol 944 server pool")
	}
	if _, ok := protocolVersion.Packets(true)[packet.IDUpdateBlock]; ok {
		t.Fatal("UpdateBlock is present in protocol 944 client pool")
	}
	for _, id := range []uint32{packet.IDClientboundUpdateSoundData, packet.IDSendPartyDestinationCookie, packet.IDPartyDestinationCookieResponse, packet.IDServerStoreInfo, packet.IDServerPresenceInfo} {
		if _, ok := protocolVersion.Packets(false)[id]; ok {
			t.Fatalf("post-944 packet ID %d is present in protocol 944 server pool", id)
		}
		if _, ok := protocolVersion.Packets(true)[id]; ok {
			t.Fatalf("post-944 packet ID %d is present in protocol 944 client pool", id)
		}
	}
}
...
func TestHistoricalGophertunnelOracle(t *testing.T) {
	oracle := os.Getenv("V944_WIRE_ORACLE")
	if oracle == "" {
		t.Skip("V944_WIRE_ORACLE is not set")
	}
	for _, fixture := range wireFixtures() {
		t.Run(fixture.name, func(t *testing.T) {
			pk := fixture.new()
