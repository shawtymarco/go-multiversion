...
func marshalPlayerArmourDamage827(io *wireIO, raw packet.Packet) {
	...
	for _, entry := range pk.List {
		if entry.ArmourSlot < 0 || entry.ArmourSlot >= armourSlotCount827 {
			io.InvalidValue(entry.ArmourSlot, "armour slot", "must be between 0 and 4 for protocol 827")
			return
		}
		mask := uint8(1 << entry.ArmourSlot)
		if bitset&mask != 0 {
			io.InvalidValue(entry.ArmourSlot, "armour slot", "duplicate slot cannot be represented by protocol 827")
			return
		}
		bitset |= mask
