...
func wireFixtures() []packetFixture {
	return []packetFixture{
		...
		{name: "v827/resource_pack_stack", new: func() packet.Packet {
			return &packet.ResourcePackStack{TexturePackRequired: true, TexturePacks: []protocol.StackResourcePack{{}}, BaseGameVersion: "1.21.102", Experiments: []protocol.ExperimentData{{Name: "test", Enabled: true}}, ExperimentsPreviouslyToggled: true, IncludeEditorPacks: true}
		}},
		{name: "v827/actor_event", new: func() packet.Packet {
		...
		{name: "v827/mob_effect", new: func() packet.Packet {
		...
		{name: "v827/interact", listener: true, new: func() packet.Packet {
		...
		{name: "v827/command_request", listener: true, new: func() packet.Packet {
		...
		{name: "v827/command_output", new: func() packet.Packet {
		...
		{name: "v827/available_commands_enum_index", new: func() packet.Packet {
		...
		{name: "v827/show_store_offer", new: func() packet.Packet {
		...
		{name: "v827/client_bound_debug_renderer", new: func() packet.Packet {
		...
		{name: "v827/update_client_input_locks", new: func() packet.Packet {
			return &packet.UpdateClientInputLocks{Locks: packet.ClientInputLockCamera | packet.ClientInputLockMovement}
		}},
		{name: "v827/camera_instruction_fov", new: func() packet.Packet {
		...
		{name: "v827/update_client_options", listener: true, new: func() packet.Packet {
		...
		{name: "v827/camera_aim_assist_presets", new: func() packet.Packet {
		...
		{name: "v827/book_edit", listener: true, new: func() packet.Packet {
			return &packet.BookEdit{ActionType: packet.BookActionSwapPages, InventorySlot: 2, PageNumber: 3, SecondaryPageNumber: 4}
		}},
		{name: "v827/client_cache_blob_status", listener: true, new: func() packet.Packet {
			return &packet.ClientCacheBlobStatus{MissHashes: []uint64{1, 2}, HitHashes: []uint64{3}}
		}},
		{name: "v827/structure_template", listener: true, new: func() packet.Packet {
		...
		{name: "v827/biome_definition_list", new: func() packet.Packet {
		...
		{name: "v827/event_cauldron", new: func() packet.Packet {
...
func TestProtocolIdentityAndPools(t *testing.T) {
	...
	if got, want := protocolVersion.ID(), int32(827); got != want {
		t.Fatalf("protocol ID: got %d, want %d", got, want)
	}
	if got, want := protocolVersion.Ver(), "1.21.102"; got != want {
		t.Fatalf("version: got %q, want %q", got, want)
	}
	if _, ok := protocolVersion.Packets(false)[packet.IDServerPlayerPostMovePosition]; ok {
		t.Fatal("ServerPlayerPostMovePosition is present in protocol 827 server pool")
	}
	if _, ok := protocolVersion.Packets(true)[packet.IDUpdateBlock]; ok {
		t.Fatal("UpdateBlock is present in protocol 827 client pool")
	}
	...
	if got, want := len(protocolVersion.Packets(true)), 78; got != want {
	...
	if _, ok := protocolVersion.Packets(true)[packet.IDServerBoundPackSettingChange]; ok {
		t.Fatal("ServerBoundPackSettingChange is present in protocol 827 client pool")
	}
...
func TestStableGameVersions(t *testing.T) {
	for _, version := range []string{"1.21.100", "1.21.101", "1.21.102"} {
		if !isStableGameVersion(version) {
			t.Fatalf("stable family version %s was rejected", version)
		}
	}
	for _, version := range []string{"1.21.99", "1.21.103", "1.21.110", "1.26.45", ""} {
		if isStableGameVersion(version) {
...
func TestHistoricalEntityFlagCount(t *testing.T) {
	if got, want := entityDataFlagCount827, 125; got != want {
		t.Fatalf("entity flag count: got %d, want %d", got, want)
	}
	if got, want := len(soundEvents827), 563; got != want {
...
func TestHistoricalGophertunnelOracle(t *testing.T) {
	oracle := os.Getenv("V827_WIRE_ORACLE")
	if oracle == "" {
		t.Skip("V827_WIRE_ORACLE is not set")
	}
	for _, fixture := range wireFixtures() {
		t.Run(fixture.name, func(t *testing.T) {
			pk := fixture.new()
