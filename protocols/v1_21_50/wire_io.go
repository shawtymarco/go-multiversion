...
// operations whose protocol-766 layout differs from native.
type wireIO struct {
	protocol.IO
	reading bool
	runtime *runtimeData
...
func (io *wireIO) SoundPos(x *mgl32.Vec3) {
	position := protocol.BlockPos{int32((*x)[0] * 8), int32((*x)[1] * 8), int32((*x)[2] * 8)}
	io.UBlockPos(&position)
	*x = mgl32.Vec3{float32(position[0]) / 8, float32(position[1]) / 8, float32(position[2]) / 8}
}
...
func (io *wireIO) GameRule(x *protocol.GameRule) { marshalGameRule(io, x, true) }
