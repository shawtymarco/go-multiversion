...
func wireFixtures() []packetFixture {
	return []packetFixture{
		...
		{name: "v766/resource_pack_stack", new: func() packet.Packet {
			return &packet.ResourcePackStack{TexturePackRequired: true, TexturePacks: []protocol.StackResourcePack{{}}, BaseGameVersion: "1.21.51", Experiments: []protocol.ExperimentData{{Name: "test", Enabled: true}}, ExperimentsPreviouslyToggled: true, IncludeEditorPacks: true}
		}},
		{name: "v766/actor_event", new: func() packet.Packet {
		...
		{name: "v766/mob_effect", new: func() packet.Packet {
		...
		{name: "v766/interact", listener: true, new: func() packet.Packet {
		...
		{name: "v766/command_request", listener: true, new: func() packet.Packet {
		...
		{name: "v766/command_output", new: func() packet.Packet {
		...
		{name: "v766/available_commands_enum_index", new: func() packet.Packet {
		...
		{name: "v766/show_store_offer", new: func() packet.Packet {
		...
		{name: "v766/client_bound_debug_renderer", new: func() packet.Packet {
		...
		{name: "v766/update_client_input_locks", new: func() packet.Packet {
			return &packet.UpdateClientInputLocks{Locks: packet.ClientInputLockCamera | packet.ClientInputLockMovement}
		}},
		{name: "v766/camera_aim_assist_presets", new: func() packet.Packet {
		...
		{name: "v766/book_edit", listener: true, new: func() packet.Packet {
			return &packet.BookEdit{ActionType: packet.BookActionSwapPages, InventorySlot: 2, PageNumber: 3, SecondaryPageNumber: 4}
		}},
		{name: "v766/client_cache_blob_status", listener: true, new: func() packet.Packet {
			return &packet.ClientCacheBlobStatus{MissHashes: []uint64{1, 2}, HitHashes: []uint64{3}}
		}},
		{name: "v766/structure_template", listener: true, new: func() packet.Packet {
		...
		{name: "v766/biome_definition_list", new: func() packet.Packet {
		...
		{name: "v766/event_cauldron", new: func() packet.Packet {
...
func TestProtocolIdentityAndPools(t *testing.T) {
	...
	if got, want := protocolVersion.ID(), int32(766); got != want {
		t.Fatalf("protocol ID: got %d, want %d", got, want)
	}
	if got, want := protocolVersion.Ver(), "1.21.51"; got != want {
		t.Fatalf("version: got %q, want %q", got, want)
	}
	if _, ok := protocolVersion.Packets(false)[packet.IDServerPlayerPostMovePosition]; ok {
		t.Fatal("ServerPlayerPostMovePosition is present in protocol 766 server pool")
	}
	if _, ok := protocolVersion.Packets(true)[packet.IDUpdateBlock]; ok {
		t.Fatal("UpdateBlock is present in protocol 766 client pool")
	}
	if _, ok := protocolVersion.Packets(true)[packet.IDServerBoundPackSettingChange]; ok {
		t.Fatal("ServerBoundPackSettingChange is present in protocol 766 client pool")
	}
...
func TestStableGameVersions(t *testing.T) {
	for _, version := range []string{"1.21.50", "1.21.51"} {
		if !isStableGameVersion(version) {
			t.Fatalf("stable family version %s was rejected", version)
		}
	}
	for _, version := range []string{"1.21.49", "1.21.52", "1.21.100", "1.26.45", ""} {
		if isStableGameVersion(version) {
...
func TestHistoricalGophertunnelOracle(t *testing.T) {
	oracle := os.Getenv("V766_WIRE_ORACLE")
	if oracle == "" {
		t.Skip("V766_WIRE_ORACLE is not set")
	}
	for _, fixture := range wireFixtures() {
		t.Run(fixture.name, func(t *testing.T) {
			pk := fixture.new()
