...
type runtimeData struct {
	...
	creative    []v766.CreativeItem
	furnaces    []v766.FurnaceRecipe
...
func newRuntimeData(native mapping.BlockRegistry, nativeItems []protocol.ItemEntry) (*runtimeData, error) {
	historicalStates, err := v766.BlockStates()
	if err != nil {
		return nil, err
	}
	...
	historicalItems, err := v766.Items()
	if err != nil {
		return nil, err
	}
	...
	creative, err := v766.Creative()
	if err != nil {
		return nil, err
	}
	furnaces, err := v766.Furnaces()
	if err != nil {
		return nil, err
	}
...
func (data *runtimeData) itemMapper(native []protocol.ItemEntry) (*mapping.ItemMapper, error) {
	if data == nil {
		return nil, fmt.Errorf("protocol 766 runtime data is not configured")
	}
	data.itemsMu.RLock()
	items := data.items
