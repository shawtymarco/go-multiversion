...
func downgradePacket(pk packet.Packet, conn *minecraft.Conn) []packet.Packet {
	...
	case *packet.GraphicsOverrideParameter:
		if current.ParameterType >= protocol.GraphicsOverrideParameterTypeHighlightsGain {
			return nil
		}
	case *packet.ClientBoundDataDrivenUICloseScreen:
		if _, targeted := current.FormID.Value(); targeted {
			return nil
		}
...
func isStableGameVersion(version string) bool {
	switch version {
	case "1.26.0", "1.26.1", "1.26.2", "1.26.3":
		return true
	default:
		return false
	}
