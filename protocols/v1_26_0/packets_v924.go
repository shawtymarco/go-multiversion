...
func marshalGraphicsOverrideParameter(io *wireIO, raw packet.Packet) {
	...
	floatValue, _ := pk.FloatValue.Value()
	io.Float32(&floatValue)
	vec3Value, _ := pk.Vec3Value.Value()
	io.Vec3(&vec3Value)
	io.String(&pk.BiomeIdentifier)
	io.Uint8(&pk.ParameterType)
	io.Bool(&pk.Reset)
	if io.reading {
		pk.FloatValue = protocol.Option(floatValue)
		pk.Vec3Value = protocol.Option(vec3Value)
...
func marshalClientBoundDataStore(io *wireIO, raw packet.Packet) {
	...
	protocol.FuncIOSlice(io.directional(), &pk.Updates, func(raw protocol.IO, entry *protocol.DataStoreChangeEntry) {
		...
		case protocol.DataStoreChangeTypeUpdate:
			marshalDataStoreUpdate924(legacy, &entry.Update)
		case protocol.DataStoreChangeTypeChange:
			...
			marshalDataStorePropertyValue924(legacy, &entry.Change.NewValue)
...
func marshalDataStoreUpdate924(io *wireIO, value *protocol.DataStoreUpdate) {
...
func marshalServerBoundDataStore(io *wireIO, raw packet.Packet) {
	marshalDataStoreUpdate924(io, &raw.(*packet.ServerBoundDataStore).Update)
}

func marshalDataStorePropertyValue924(io *wireIO, value *protocol.DataStorePropertyValue) {
	io.Int32(&value.Type)
	switch value.Type {
	...
	case protocol.DataStorePropertyTypeMap:
		protocol.FuncIOSlice(io.directional(), &value.MapValue, func(raw protocol.IO, entry *protocol.DataStoreMapEntry) {
			...
			marshalDataStorePropertyValue924(legacy, &entry.Value)
		})
	default:
...
func marshalAttributeLayer924(io *wireIO, layer *protocol.AttributeLayerData) {
	io.String(&layer.Name)
	io.Varint32(&layer.DimensionID)
	marshalAttributeLayerSettings924(io, &layer.Settings)
