// Package v898 exposes the exact historical Dragonfly registry snapshots for
// Minecraft 1.21.13x/protocol 898 using typed production-compatible decoders.
package v898
...
// FurnaceRecipe is one historical furnace recipe.
...
// Furnace decodes historical furnace recipes.
func Furnace() ([]FurnaceRecipe, error) {
	var data []FurnaceRecipe
	if err := nbt.Unmarshal(furnaceData, &data); err != nil {
		return nil, fmt.Errorf("decode furnace recipes: %w", err)
	}
	return data, nil
}

// Smithing decodes the exact historical smithing-transform recipe snapshot.
func Smithing() ([]ShapelessRecipe, error) {
	var data []ShapelessRecipe
	if err := nbt.Unmarshal(smithingData, &data); err != nil {
		return nil, fmt.Errorf("decode smithing recipes: %w", err)
	}
	return data, nil
}
